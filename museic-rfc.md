...
**Recommendations don't use a trained model.** At a ~50-track catalog there's no meaningful cold-start problem, and any model trained on the small amount of reaction data collected during a single event would be shakier than the content-based similarity scoring above — simpler is the correct engineering choice here, not just the faster one.
